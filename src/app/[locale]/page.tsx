...
import {
  Coins, Trophy, TrendingUp, UserPlus, Target, Zap,
  Calendar, ChevronRight, Medal, Award, ArrowRight, Shield, BarChart3,
} from "lucide-react";
...
export default function HomePage() {
  ...
  useEffect(() => {
    api.get<BetEvent[]>("/events/featured").then(setFeatured).catch(() => {});
    api.get<LeaderboardEntry[]>("/leaderboard?limit=5").then(setLeaders).catch(() => {});
  }, []);
    ...
    <div className="flex flex-col">
      ...
      <section className="py-24 md:py-32 lg:py-40">
        <div className="mx-auto max-w-4xl px-4 text-center">
          ...
          <div className="flex flex-col items-center gap-3 sm:flex-row sm:justify-center">
            ...
            <Link href="/events">
              <Button variant="outline" size="lg" className="gap-2 px-8 h-12 text-sm font-semibold">
                {tNav("events")}
          ...
          <div className="mt-16 grid grid-cols-3 gap-8 max-w-md mx-auto">
            {[
              ...
              { value: "50+", label: t("statsDailyBonus") },
              { value: "4", label: t("statsRoles") },
            ].map((stat, i) => (
      ...
      <section className="py-20 md:py-28 border-t border-border/50">
        <div className="mx-auto max-w-5xl px-4">
          <div className="text-center mb-14">
            ...
            <h2 className="text-2xl font-bold md:text-4xl">{t("howItWorks")}</h2>
          </div>
            ...
            {[
              ...
              <div key={i} className="rounded-lg border border-border bg-card p-6">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <span className="text-xs font-bold text-muted-foreground">{t("stepLabel", { number: i + 1 })}</span>
                </div>
        ...
        <div className="mx-auto max-w-5xl px-4">
          <div className="flex items-end justify-between mb-10">
            <div>
              ...
              <h2 className="text-2xl font-bold md:text-4xl">{t("featuredEvents")}</h2>
            </div>
            <Link href="/events">
          ...
          <div className="grid gap-4 md:grid-cols-3">
            {(featured.length > 0 ? featured.slice(0, 6) : [null, null, null]).map((event, i) => (
              <div
                key={event?.id || i}
              ...
              >
                {event ? (
                  <>
                    ...
                    {event.category && (
                      <p className="text-xs text-muted-foreground mb-3">{event.category.name}</p>
                    )}
                      ...
                      <div className="flex flex-col gap-0.5">
                        ...
                        {event.endTime && (
                          <span className="flex items-center gap-1 text-muted-foreground/70">
                            <Calendar className="h-3 w-3" />
                            {t("until")} {new Date(event.endTime).toLocaleDateString(locale, {
                              day: "2-digit", month: "short", hour: "2-digit", minute: "2-digit",
                            })}
                          </span>
                        )}
      ...
      <section className="py-20 md:py-28 border-t border-border/50">
        <div className="mx-auto max-w-5xl px-4">
          <div className="text-center mb-14">
            ...
            <h2 className="text-2xl font-bold md:text-4xl">{t("allYouNeed")}</h2>
          </div>

          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {[
              { icon: BarChart3, title: t("featLiveOddsTitle"), desc: t("featLiveOddsDesc") },
              { icon: Coins, title: t("featTokenTitle"), desc: t("featTokenDesc") },
              { icon: Shield, title: t("featSecureTitle"), desc: t("featSecureDesc") },
              { icon: Zap, title: t("featRealtimeTitle"), desc: t("featRealtimeDesc") },
            ].map((feat, i) => (
        ...
        <div className="mx-auto max-w-3xl px-4">
          <div className="flex items-end justify-between mb-8">
            <div>
              ...
              <h2 className="text-2xl font-bold md:text-4xl">{t("topLeaderboard")}</h2>
            </div>
              ...
              leaders.map((entry) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-6 text-sm shrink-0">
                    <span className="text-muted-foreground hidden sm:inline">{entry.totalBets} {tLb("bets")}</span>
                    <span className={`font-semibold tabular-nums ${entry.profit >= 0 ? "text-emerald-500" : "text-red-500"}`}>
      ...
      <section className="py-20 md:py-28 border-t border-border/50">
        <div className="mx-auto max-w-2xl px-4 text-center">
          <h2 className="text-2xl font-bold md:text-4xl mb-3">{t("readyToBet")}</h2>
