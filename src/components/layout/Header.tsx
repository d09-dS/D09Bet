...
import { Coins, Menu, User, LogOut, Settings, TicketCheck, X } from "lucide-react";
    ...
    <header className="sticky top-0 z-50 bg-background/95 backdrop-blur-sm border-b border-border">
      <div className="mx-auto flex h-14 max-w-7xl items-center justify-between px-4">
        ...
        <Link href="/" className="flex items-center opacity-90 hover:opacity-100 transition-opacity">
          <Image src="/dotbet_logo.png" alt="dotBet" width={110} height={36} className="h-9 w-auto object-contain" priority />
        </Link>
        ...
        <div className="flex items-center gap-2">
          ...
          {session?.user ? (
            <>
              ...
              <DropdownMenu>
                <DropdownMenuTrigger render={
                  <Button variant="ghost" size="icon" className="rounded-full h-9 w-9 border border-border hover:border-primary/50 transition-colors" />
                }>
            ...
            <div className="hidden gap-2 sm:flex">
              ...
              <Link href="/register">
                <Button size="sm" className="rounded-lg">{t("register")}</Button>
              </Link>
            </div>
          )}
        ...
        {mobileOpen && (
          ...
          >
            <nav className="flex flex-col gap-1 p-4">
              ...
              {session?.user ? (
                <>
                  <Link href="/profile" onClick={() => setMobileOpen(false)} className="flex items-center gap-3 rounded-lg px-4 py-3 text-sm font-medium hover:bg-white/[0.04]">
                  ...
                  <Link href="/settings" onClick={() => setMobileOpen(false)} className="flex items-center gap-3 rounded-lg px-4 py-3 text-sm font-medium hover:bg-white/[0.04]">
                    <Settings className="h-4 w-4 text-muted-foreground" />
                    {t("settings")}
                  </Link>
                  {(session.user.role === "ADMIN" || session.user.role === "MODERATOR") && (
                    <Link href="/admin" onClick={() => setMobileOpen(false)} className="flex items-center gap-3 rounded-lg px-4 py-3 text-sm font-medium hover:bg-white/[0.04]">
                      <Settings className="h-4 w-4 text-muted-foreground" />
                      {t("admin")}
                    </Link>
                  )}
